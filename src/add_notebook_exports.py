"""
Script to add figure and metadata export functionality to pola_svm_classification.ipynb
"""
import json
import os
...
SAVE_OUTPUTS_CELL = {
    ...
    "source": [
        ...
        "plt.ylabel('Actual')\n",
        "plt.title(f'Confusion Matrix - NCA 9 + SVM (RBF)')\n",
        "plt.tight_layout()\n",
        "plt.savefig(f'{figures_dir}/confusion_matrix_nca9.png', dpi=150, bbox_inches='tight')\n",
        "plt.close()\n",
        "print(f'Saved: {figures_dir}/confusion_matrix_nca9.png')\n",
        "\n",
        "# --- Save Cross-Validation Scores Plot ---\n",
        "plt.figure(figsize=(10, 6))\n",
        "plt.plot(range(1, 11), cv_scores, 'o-', linewidth=2, markersize=8, color='steelblue')\n",
        ...
        "plt.tight_layout()\n",
        "plt.savefig(f'{figures_dir}/cv_scores_nca9.png', dpi=150, bbox_inches='tight')\n",
        "plt.close()\n",
        "print(f'Saved: {figures_dir}/cv_scores_nca9.png')\n",
        "\n",
        ...
        "ax.bar(x + width, metrics_df['f1-score'], width, label='F1-Score', color='forestgreen')\n",
        "ax.set_ylabel('Score')\n",
        ...
        "ax.set_ylim([0, 1])\n",
        "plt.tight_layout()\n",
        "plt.savefig(f'{figures_dir}/per_class_metrics_nca9.png', dpi=150, bbox_inches='tight')\n",
        "plt.close()\n",
        "print(f'Saved: {figures_dir}/per_class_metrics_nca9.png')\n",
        "\n",
        "# --- Save Metadata JSON ---\n",
        ...
        "}\n",
        "\n",
        "with open(f'{reports_dir}/nca9_svm_metadata.json', 'w') as f:\n",
        "    json.dump(metadata, f, indent=2)\n",
        "print(f'Saved: {reports_dir}/nca9_svm_metadata.json')\n",
        "\n",
...
def add_export_cell():
    ...
    for cell in nb['cells']:
        if cell.get('id') == 'save_outputs_cell':
            print("Export cell already exists, skipping.")
            return
    
    # Add the new cell before the last cell (or at the end)
